...
pub const struct_BebopCtx = opaque {};
pub const BebopCtx = struct_BebopCtx;
pub const struct_VBytes = extern struct {
    ptr: [*c]const u8 = @import("std").mem.zeroes([*c]const u8),
    len: usize = @import("std").mem.zeroes(usize),
};
pub const VBytes = struct_VBytes;
pub const struct_VSensorReading = extern struct {
    timestamp: u64 = @import("std").mem.zeroes(u64),
    sensor_id: VBytes = @import("std").mem.zeroes(VBytes),
    sensor_type: u16 = @import("std").mem.zeroes(u16),
    value: f64 = @import("std").mem.zeroes(f64),
    unit: VBytes = @import("std").mem.zeroes(VBytes),
    location: VBytes = @import("std").mem.zeroes(VBytes),
    metadata_count: usize = @import("std").mem.zeroes(usize),
    metadata_keys: [*c]VBytes = @import("std").mem.zeroes([*c]VBytes),
    metadata_values: [*c]VBytes = @import("std").mem.zeroes([*c]VBytes),
    error_code: i32 = @import("std").mem.zeroes(i32),
    error_message: [*c]const u8 = @import("std").mem.zeroes([*c]const u8),
};
pub const VSensorReading = struct_VSensorReading;
pub extern fn bebop_ctx_new() ?*BebopCtx;
pub extern fn bebop_ctx_free(ctx: ?*BebopCtx) void;
pub extern fn bebop_ctx_reset(ctx: ?*BebopCtx) void;
pub extern fn bebop_decode_sensor_reading(ctx: ?*BebopCtx, data: [*c]const u8, len: usize, out: [*c]VSensorReading) i32;
pub extern fn bebop_free_sensor_reading(ctx: ?*BebopCtx, reading: [*c]VSensorReading) void;
pub extern fn bebop_encode_batch_readings(ctx: ?*BebopCtx, readings: [*c]const VSensorReading, count: usize, out_buf: [*c]u8, out_len: usize) usize;
...
// /var/home/hyper/.opsm/runtimes/zig/0.15.2/lib/include/__stddef_offsetof.h:16:9
